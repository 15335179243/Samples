...
import com.eyecool.visitor.sys.bean.ComBean;
import com.eyecool.visitor.sysinner.App;
...
public class PrinterUtil {
    ...
    public boolean OpenComPort() {
        try {
            ...
            ToastUtil.showToast(App.appInstance,"没有读写权限！");
            isOpenCom = false;
            return isOpenCom;
        } catch (IOException e) {
            ToastUtil.showToast(App.appInstance,"io流错误！");
            isOpenCom = false;
            return isOpenCom;
        } catch (InvalidParameterException e) {
            ToastUtil.showToast(App.appInstance,"参数错误！");
            isOpenCom = false;
            return isOpenCom;
        }
    }
    ...
    public void printEyeCoolInfo() {
        if (comA.isOpen()) {
            ...
            ToastUtil.showToast(App.appInstance,"请打开打印机！");
            return;
        }
    }
    ...
    public void printTest(String strTitle, int order,String visitorName,String receiveTeacherName,String techerPhoneNum,int gender) {
        if (comA.isOpen()) {
            ...
            comA.send(PrintCmd.PrintString((gender == 0 ? "先生" : "女士") + "!", 0));


            comA.send(PrintCmd.PrintFeedline(2));
            comA.send(PrintCmd.SetAlignment(0));
            comA.send(PrintCmd.PrintString("您的接待老师：", 0));
